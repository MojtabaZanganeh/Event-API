...
class zarinpal
{
	...
	public function request($Amount, $CallbackURL, $Description = "", $Email = "", $Mobile = "", $SandBox = false, $ZarinGate = false)
	{
	...
	public function verify($Authority, $Amount, $SandBox = false, $ZarinGate = false)
	{
		$ZarinGate = ($SandBox == true) ? false : $ZarinGate;
		...
		if ($this->soap_check() === true) {
			...
			$data = array('MerchantID' => $this->MerchantID, 'Authority' => $au, 'Amount' => $Amount);
			$jsonData = json_encode($data);
